...
class Browser():
    ...
    def analysis_html(self):
        ...
        while new_count > old_count:
            ...
            print "this time got %dth data in fact!"%(len(append_list))
    ...
    def get_content(self):
        #采集一条tweets的所有数据
        #t_texts = driver.find_elements_by_class_name('expanding-stream-item')
    ...
    def generate(self):
        ...
        for months in range(int(d1[1]),int(d2[1])):
            for day in range(int(d1[2]),calendar.monthrange(int(d1[0]), months)[1]):
                ...
                s1="https://twitter.com/search?q="
                s2=self.keywords.get()
                s3="%20lang%3A"
                s4=self.lan.get()
                s5="%20since%3A"
                s6="%s-%d-%d" %(d1[0],months,day)
                s7="%20until%3A"
                s8="%s-%d-%d&src=typd"%(d1[0],months,day+1)
        ...
        for day in range(int(d1[2]),int(d2[2])):
            ...
            s4=self.lan.get()
            s5="%20since%3A"
            s6="%s-%s-%d" %(d1[0],d2[1],day)
            s7="%20until%3A"
            s8="%s-%s-%d&src=typd"%(d1[0],d2[1],day+1)
            s=s1+s2+s3+s4+s5+s6+s7+s8
            urllist.append(s)
        return urllist
    def start(self):
